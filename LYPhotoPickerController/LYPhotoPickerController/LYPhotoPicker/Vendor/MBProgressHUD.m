...
static const void *kLargeHUD  = &kLargeHUD;
static const void *kSmallHUD  = &kSmallHUD;

@implementation  MBProgressHUD (Customer)

+ (void)showLargeHUD:(NSString *)title {
    NSAssert([NSThread isMainThread], @"MBProgressHUD needs to be accessed on the main thread.");
    
    MBProgressHUD *smallHUD = objc_getAssociatedObject(self, kSmallHUD);
    if (smallHUD && smallHUD.alpha) {
        [smallHUD hide:YES];
    }
    
    MBProgressHUD *largeHUD = objc_getAssociatedObject(self, kLargeHUD);
    if (!largeHUD) {
        largeHUD = [[MBProgressHUD alloc] initWithView:[UIApplication sharedApplication].keyWindow];
        largeHUD.removeFromSuperViewOnHide = YES;
        largeHUD.minSize = CGSizeMake(100, 100);
        objc_setAssociatedObject(self, kLargeHUD, largeHUD, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
    
    largeHUD.labelText = title;
    [[UIApplication sharedApplication].keyWindow addSubview:largeHUD];
    [largeHUD show:YES];
}
...
+ (void)showSmallHUD {
    ...
    MBProgressHUD *largeHUD = objc_getAssociatedObject(self, kLargeHUD);
    if (largeHUD && largeHUD.alpha) {
        [largeHUD hide:YES];
    }
    
    MBProgressHUD *smallHUD = objc_getAssociatedObject(self, kSmallHUD);
    if (!smallHUD) {
        smallHUD = [[MBProgressHUD alloc] initWithView:[UIApplication sharedApplication].keyWindow];
        smallHUD.mode = MBProgressHUDModeCustomView;
        smallHUD.removeFromSuperViewOnHide = YES;
        smallHUD.color = [UIColor clearColor];
        ...
        smallHUD.customView = customView;
        
        objc_setAssociatedObject(self, kSmallHUD, smallHUD, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
    
    [[UIApplication sharedApplication].keyWindow addSubview:smallHUD];
    [smallHUD show:YES];
}
...
+ (void)dismissHUDAferDelay:(NSInteger)seconds {
    NSAssert([NSThread isMainThread], @"MBProgressHUD needs to be accessed on the main thread.");
    MBProgressHUD *largeHUD = objc_getAssociatedObject(self, kLargeHUD);
    if (largeHUD && largeHUD.alpha) {
        [largeHUD performSelector:@selector(hide:) withObject:@YES afterDelay:seconds];
    }
    
    MBProgressHUD *smallHUD = objc_getAssociatedObject(self, kSmallHUD);
    if (smallHUD && smallHUD.alpha) {
        [smallHUD performSelector:@selector(hide:) withObject:@YES afterDelay:seconds];
    }
}
